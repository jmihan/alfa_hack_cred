...
`make_submission` собирает CSV `request_id;variant_no;score`, сверяясь со схемой
`commit.csv`; `verify_submission` проверяет соответствие набора ключей эталону;
`apply_pil1mtrx_hard_rule` гарантирует, что оффер с `pil1mtrx_offer=1` встаёт на
первое место в своей заявке (он становится сделкой в 99.7% случаев).
"""
...
def apply_pil1mtrx_hard_rule(
    ...
    После такого «буста» оффер с pil1mtrx=1 гарантированно окажется на первом
    месте в своей группе при сортировке по убыванию score. Согласно EDA, при
