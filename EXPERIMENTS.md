...
**Воспроизводимость — два режима.**

- **`reproduce`** ([`scripts/reproduce_record.py`](scripts/reproduce_record.py)) —
  **байт-в-байт** рекорд 92.1957. A-сторона record_11 и B-ранги (b_blend + MLP)
  зафиксированы как предсказания на test в [`artifacts/record/`](artifacts/record);
  скрипт детерминированно собирает из них сабмит (`0.70·b_blend + 0.30·MLP` на B,
  record_11 на A; порядок строк берётся из данных через `sort_by_request`) и сверяет
  sha256 с эталоном. Работает на любой машине, GPU не нужен. A-база record_11 (агрегат
  десятков прогонов) и нейросеть невоспроизводимы переобучением побайтно, поэтому
  именно зафиксированные предсказания дают точный рекорд.
- **`train`** ([`scripts/fit_pipeline.py`](scripts/fit_pipeline.py)) + **`inference`**
  ([`scripts/predict.py`](scripts/predict.py)) — обучение близкого пайплайна с нуля и
  сборка из сохранённых моделей; сборка обеих стадий — в
  [`two_stage.py`](src/alfa_cred/two_stage.py). GBDT детерминированы (бит-в-бит при
  тех же сидах/данных/машине), MLP — с фиксированными сидами (run-to-run воспроизводим
  в одной среде; принудительную однопоточность на CPU убрали — она смещала результат
  до ~92.17). Между средами (CPU/GPU, версии torch) нейросеть не бит-в-бит, итог
  плавает в ~±0.002: from-scratch на CPU (Docker, Ryzen 5 7500F) = **92.1918**.
  GPU-обучение (XGBoost/CatBoost/MLP; LightGBM на CPU) — образ `Dockerfile.gpu`.
  NDCG@5 зависит только от порядка офферов, поэтому метрика устойчива на уровне ~92.19.
...
**Итог.** Лучший результат остаётся за основным пайплайном — **LB 92.1957**. Ни
дополнительный feature engineering, ни псевдолейблинг разрыв с лидерами не закрывают;
их преимущество, вероятно, во внешнем сигнале/данных за пределами предоставленного
набора.
