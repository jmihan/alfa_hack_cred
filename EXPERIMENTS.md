...
Лучший public-результат — **LB 92.1957** (широкий offer-набор + `is_best_both` +
pointwise-MLP), но на **приватном** лидерборде он просел; лучшим на привате оказался
более простой **`two_stage_record11_plus_bAllL`** (public 92.0006) — см. секцию
«Приватный лидерборд» в конце. Именно его воспроизводит финальная версия репозитория.
...
**Цель.** Лидерборд показал 92.55, мой потолок ~92.05. Подозрение, что
есть какой-то сигнал в данных, который я не использую. Поискать через:
...
**Вывод.** «Скрытого сигнала» в данных нет (или я не научился его
вынимать). Лидер 92.55 вероятно использует внешние данные или
специфический трюк, который мне повторить не удалось.
...
основному B-blend, не как замена. Pairwise binary classification в
этой постановке не работает (tournament не складывается в линейный
порядок).
...
**B-бленд** (`b_blend`; модуль удалён при финализации — в приватный рекорд не входит):
...
**Модель** (`mlp_pointwise`; модуль удалён при финализации — в приватный рекорд не входит):
...
**CV-LB разрыв (снова с недооценкой LB).** На honest B-сплите прирост в OOF едва
...
**A-сторона.** Рекордный сабмит (92.1957) использует историческую 11-модельную
A-базу `record_11` (`two_stage_record11_plus_bBalanced_isbb`). Для обучения с нуля
A-сторона заменена компактным 5-модельным A-блендом
([`models/a_blend.py`](src/alfa_cred/models/a_blend.py): 3×LGBM extended +
2×CatBoost YetiRank). Top-1 на test совпадает 1:1 (hard-rule доминирует на
A-заявках), но ранги на нижних позициях чуть отличаются, поэтому from-scratch с
компактной A даёт **92.1918** против 92.1957 у record_11.

**Воспроизводимость (на момент public-фазы).** Режим `reproduce` тогда собирал
байт-в-байт public-рекорд 92.1957 из зафиксированных предсказаний (record_11 +
b_blend + MLP), а `train`/`inference` обучали близкий пайплайн с нуля — 92.1918 на CPU
(Docker, Ryzen 5 7500F); нейросеть между средами не бит-в-бит, итог плавал в ~±0.002.
После подсчёта приватного лидерборда оба режима переориентированы на приватный рекорд
bAllL — см. «Финальное состояние репозитория» в конце.

## Итоговый сабмит фазы M-P (2026-05-26, 14:05)
...
|----------------|----|------------------|
| candidate_selfcontained (pointwise-MLP) | **92.1957** | A=наш + B=0.70·b_blend+0.30·MLP — public-рекорд, на привате просел |
...
| two_stage_record11_plus_bAllL | **92.0006** | 5 XGB Optuna + 1 LGBM ext (Pipeline L) — **лучший на привате** |
...
## Финальная фаза: поиск дополнительного сигнала в подзадаче B

После фиксации рекорда (LB 92.1957) разрыв с лидерами при потолке подзадачи A
(hard-rule даёт ≈0.998) локализован в подзадаче B. Проверяли гипотезу, что недобор —
в feature engineering, на валидации, близкой к LB: **5-fold GroupKFold по
`request_id` на B-only + time-holdout** (ранние→поздние заявки), бленд LGB+XGB+CB,
порог значимости ΔNDCG@5 ≈ 0.004. Базовый B-NDCG@5 ≈ **0.758**.

**Скрининг признаков (~52 гипотезы) — значимых нет.**
- Дискретные композиты из атрибутов оффера/заявки (best-limit/term, relaxed-match,
  second-both, экстремумы по rate/limit/ncl/eva, парето-композиты) — в пределах
  фолд-шума (|Δ| ≤ 0.001).
- Клиент×оффер кросс-фичи (limit/баланс, платёж/доход и т.п.) и OOF target-encoding
  ключей (`offer_type`, `basket_name`, `risk`, `channel` + персонализация
  `offer_type×clientsegment`, трёхуровневые) — тоже шум.
- OOF target-encoding по `offer_id` (95% пересечения train↔test, ~915 продуктов;
  `offer_id` намеренно исключён из фич как ID) — **значимо ухудшает** (−0.006 CV):
  продуктовый сигнал уже извлекается моделью через атрибуты оффера, явный
  per-product приор лишь добавляет переобучение.

Вывод: на доступных данных пространство признаков насыщено — GBDT уже извлекает
имеющийся сигнал, инженерия признаков разрыв не закрывает.

**Псевдолейблинг (2 финальных сабмита).** Обучение на всём train-B → разметка топ-1
в уверенной части test-B (40% / 70% заявок по кросс-заявочному скору) → дообучение на
train-B + псевдоразметке → пере-предсказание; A = record_11, B = 0.70·pseudo_b_blend
+ 0.30·MLP.

| сабмит | LB |
|--------|----|
| `pseudo_conf40` (разметка 40% заявок) | 92.1196 |
| `pseudo_conf70` (разметка 70% заявок) | 92.1073 |

Оба **ниже рекорда 92.1957**: псевдоразметка усиливает собственные смещения модели и
добавляет шум, прироста на ранжировании не даёт.

**Итог public-фазы.** Лучший public-результат — основной пайплайн, **LB 92.1957**. Ни
дополнительный feature engineering, ни псевдолейблинг разрыв с лидерами на public не
закрывают.

## Приватный лидерборд: public-рекорд переобучился

После закрытия хакатона посчитали **приватный** лидерборд — и картина изменилась:
public-рекорд 92.1957 (8-модельный B-бленд + pointwise-MLP) на привате **просел**, а
лучшим из моих сабмитов на привате оказался **`two_stage_record11_plus_bAllL`**
(public LB 92.0006):

- A-сторона: record_11 + hard-rule (та же, что у public-рекорда);
- B-сторона `bAllL`: rank-avg 5×XGBoost (Optuna-параметры, сиды 42/314/8848/137/2026)
  + 1×LightGBM extended — простой multi-seed ансамбль без NN.

**Урок (public/private overfitting).** Прирост MLP-диверсити (+0.018 на public) был
подгонкой под публичную часть — на привате он не воспроизвёлся. Простой устойчивый
multi-seed XGB-ансамбль обобщил лучше сложного «подогнанного под public» бленда. Стоило
сильнее доверять простоте и устойчивости, а не приросту на public.

**Финальное состояние репозитория.** Каноническим воспроизводимым решением сделан
`bAllL`: режим `reproduce` (`scripts/reproduce_record.py`) собирает его байт-в-байт из
зафиксированных предсказаний (`artifacts/record/`: A record_11 + B bAllL) со сверкой
sha256; режим `train` (`scripts/fit_pipeline.py`) обучает B-сторону с нуля (5 XGBoost +
1 LightGBM) + компактный A-бленд.
