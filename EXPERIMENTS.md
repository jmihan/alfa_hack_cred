...
GBDT-часть детерминирована (бит-в-бит при тех же сидах/данных/машине), MLP — с
фиксированными сидами + `use_deterministic_algorithms` + однопоточностью на CPU
(`set_num_threads(1)`), поэтому `train` даёт один и тот же результат при каждом
прогоне в одной среде. Но **между средами** (CPU vs GPU, разные версии torch)
нейросеть не воспроизводится бит-в-бит, поэтому итог плавает в пределах ~±0.002:
воспроизводимый с нуля результат на CPU (Docker, Ryzen 5 7500F) = **92.1918**, а
лучший аплоад 92.1957 получен с 11-модельной A-базой + GPU-обучением MLP. NDCG@5
зависит только от порядка офферов, поэтому метрика устойчива на уровне ~92.19.
