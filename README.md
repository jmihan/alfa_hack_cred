...
CPU-образ ([`Dockerfile`](Dockerfile)) собирает Python 3.11 + зависимости + пакет
(`pip install -e .`, torch — CPU-сборка) и кладёт внутрь зафиксированные предсказания
рекорда (`artifacts/record`). Данные монтируются как volume (`./data`, `./models`,
`./submissions`, ...). Режимы — сервисы [`docker-compose.yml`](docker-compose.yml):
