...
> Воспроизводимый с нуля результат пайплайна — **≈ 92.19** (на Ryzen 5 7500F, CPU:
> 92.1918). Лучший одиночный аплоад на лидерборде — **92.1957** (тот же подход, но с
> 11-модельной A-базой и GPU-обучением MLP). Точное значение слегка зависит от среды:
> нейросеть не воспроизводится бит-в-бит между CPU/GPU и версиями torch, поэтому
> метрика плавает в пределах ~±0.002. NDCG@5 зависит только от порядка офферов.
...
### Рекордный сабмит (LB ≈ 92.19), обучение с нуля
