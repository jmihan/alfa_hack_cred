...
закрывает 8-модельный B-бленд ([`models/record_b_blend.py`](src/alfa_cred/models/record_b_blend.py)):
...
([`features/offer_v3.py`](src/alfa_cred/features/offer_v3.py)): внутригрупповые
ранги/нормализации, кросс-офферные сравнения внутри типа и риска,
Парето-доминирование и ask-match стек с ключевой фичей **`is_best_both`**.
...
│   │   ├── match.py               # ask-match признаки + is_best_both
│   │   └── offer_v3.py            # широкий offer-набор для рекордного B-бленда
│   ├── models/                    # обёртки бустингов и MLP
│   │   └── record_b_blend.py      # рекордный 8-модельный B-бленд (LGB×3+XGB×3+CB×2)
...
│   └── make_record_submission.py  # рекордный пайплайн (LB ≈ 92.18)
...
Рекордный пайплайн обучает 8-модельный B-бленд с нуля на широком offer-наборе
([`features/offer_v3.py`](src/alfa_cred/features/offer_v3.py)) и собирает
two-stage сабмит (A = record_11 + hard-rule, B = этот бленд):
...
берётся из сохранённых `oof/<record_11>_test_scores.parquet`. Главный сигнал
