...
| рекордный пайплайн (широкий offer-набор + 8-модельный B-бленд) | **≈ 92.18** ← финал |

Главный прорыв финальной фазы — фича **`is_best_both`**: оффер, точно
совпадающий с заявкой клиента (`limit == req_loan_amount` И `term == req_term`)
и первый среди таких по `variant_no`. Deal-rate такого оффера ≈ **52%** против
≈2.8% у остальных — «мягкий» аналог hard-rule, но для подзадачи B.
...
```
       ...
       в группе есть pil1mtrx_offer=1            pil1mtrx-флага нет
                  │                                         │
                  ▼                                         ▼
       rank-avg blend record_11                выделенный 8-модельный B-бленд
       (11 моделей: LGBM, CatBoost, XGB)        на широком offer-наборе (361 фича),
       + hard-rule: pil1 → верх                 обучен ТОЛЬКО на B-заявках
                  │                                         │
                  └────────────────┬────────────────────────┘
                                   │
                                   ▼
...
оффер становится сделкой в 99.7% случаев) поверх rank-avg blend `record_11`.

**Подзадача B** (заявки без pil1-оффера) — реальная задача ранжирования. Её
закрывает 8-модельный B-бленд ([`models/b_blend.py`](src/alfa_cred/models/b_blend.py)):
LightGBM LambdaRank ×3 + XGBoost rank:ndcg ×3 + CatBoost YetiRank ×2, каждая
обучается на всех train-B заявках, скоры усредняются как перцентильные ранги
внутри `request_id`. Модели видят широкий offer-набор
([`build_wide_feature_table`](src/alfa_cred/features/pipeline.py)): внутригрупповые
ранги/нормализации (см. [`features/group.py`](src/alfa_cred/features/group.py)),
кросс-офферные сравнения внутри типа и риска, Парето-доминирование и ask-match
стек с ключевой фичей **`is_best_both`** ([`features/match.py`](src/alfa_cred/features/match.py)).
...
│   ├── predict.py                 # финальный пайплайн (LB ≈ 92.18)
...
Финальный пайплайн обучает 8-модельный B-бленд с нуля на широком offer-наборе
([`build_wide_feature_table`](src/alfa_cred/features/pipeline.py)) и собирает
two-stage сабмит (A = 11-модельный A-бленд + hard-rule, B = этот бленд):

```powershell
python scripts/predict.py --out submissions/record_submission.csv
```

Обучение ~15-20 мин (LGB×3 + XGB×3 + CB×2, fit-once на train-B). A-сторона
берётся из сохранённых `oof/<model>_test_scores.parquet`. Главный сигнал
подзадачи B — фича `is_best_both` (deal-rate ≈ 52%).

Воспроизводимость ранжирования сверяется с эталоном (top-1 совпадение = 1.0):
